...
import {
  toggleWholesaleProductActive,
  deleteWholesaleProduct,
  reorderWholesaleProducts,
} from '@/lib/admin-actions';
import type { WholesaleProduct } from '@/lib/queries';

export default function ProductosWholesaleTable({
  products,
}: {
  ...
  const move = (idx: number, dir: -1 | 1) => {
    ...
    startTransition(async () => {
      setBusyId(moved.id);
      await reorderWholesaleProducts(copy.map((p) => p.id));
      setBusyId(null);
    });
  };
  ...
  const onToggle = async (id: string) => {
    ...
    await toggleWholesaleProductActive(id);
    setBusyId(null);
  };

  const onDelete = async (id: string) => {
    if (!confirm('¿Eliminar este producto mayorista?')) return;
    setBusyId(id);
      ...
      <header className="mb-6 flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 font-body text-sm text-bone/60">
            {items.length} en total. Coordinás estos pedidos por WhatsApp.
          </p>
        </div>
        <Link
          href="/admin/productos/nuevo?type=wholesale"
              ...
              >
                ...
                <td className="p-3 text-right">
                  <div className="flex justify-end gap-2">
                    <Link
                      href={`/admin/productos/${p.id}?type=wholesale`}
            ...
            {items.length === 0 && (
              <tr>
                <td colSpan={7} className="p-8 text-center text-bone/50 font-body text-sm">
                  No hay productos mayoristas todavía. Cloná uno desde Minoristas
